    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const formattedNewResults = resultNew.rows.map(row => {
                ...
                // Priorité: matricule sauvegardé > matricule depuis employees > extraction depuis notes
                ...
                return {
                    ...
                    departement: row.departement_final || row.departement || 'Département inconnu',
            ...
            // Pour les anciens enregistrements, on essaie de récupérer le matricule depuis la table employees
            // Si pas trouvé, on essaie d'extraire le matricule depuis le commentaire
            const formattedOldResults = resultOld.rows.map(row => {
                let matricule = row.matricule || null;
                
                // Si pas de matricule trouvé dans employees, essayer de l'extraire du commentaire
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            if (tableName === 'depart_history') {
                ...
                // Pour historique_departs, essayer de récupérer le matricule depuis employees
                query = `
                    SELECT 
                        hd.*,
                        e.matricule,
