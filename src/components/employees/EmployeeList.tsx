...
const EmployeeList = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      ...
      try {
        ...
        const uniqueDepartments = Array.from(new Set(data.map(emp => emp.functional_area).filter(Boolean)));
        const uniqueEntities = Array.from(new Set(data.map(emp => emp.entity).filter(Boolean)));
        const uniqueContractTypes = Array.from(new Set(data.map(emp => emp.type_contrat).filter(Boolean)));
        ...
        const uniqueDepartments = Array.from(new Set(mockEmployees.map(emp => emp.functional_area).filter(Boolean)));
        const uniqueEntities = Array.from(new Set(mockEmployees.map(emp => emp.entity).filter(Boolean)));
        const uniqueContractTypes = Array.from(new Set(mockEmployees.map(emp => emp.type_contrat).filter(Boolean)));
  ...
  useEffect(() => {
    if (employees.length > 0) {
      ...
      const uniqueDepartments = Array.from(new Set(employees.map(emp => emp.functional_area).filter(Boolean)));
      setDepartments(uniqueDepartments.map(dept => decodeHtmlEntities(dept)));
      
      // Extraire les entités uniques et les décoder
      const uniqueEntities = Array.from(new Set(employees.map(emp => emp.entity).filter(Boolean)));
      setEntities(uniqueEntities.map(entity => decodeHtmlEntities(entity)));
      
      // Extraire les types de contrat uniques et les décoder
      const uniqueContractTypes = Array.from(new Set(employees.map(emp => emp.type_contrat).filter(Boolean)));
