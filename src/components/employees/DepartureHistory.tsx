...
const DepartureHistory = () => {
  ...
  const importSchema = Yup.object().shape({
    file: Yup.mixed()
      ...
      .test('fileType', 'Format de fichier non supporté. Utilisez .xlsx, .csv ou .txt', 
        (value: any) => !value || ['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'text/csv', 'text/plain'].includes(value.type))
  });
  ...
  const fetchReferenceData = useCallback(async () => {
    try {
      ...
      const uniqueDepartments = Array.from(new Set(allDepartures
        .map(d => d.departement)
        .filter(d => d && d.trim() !== '')
      )).sort();
      
      // Extraire les types de contrat uniques
      const uniqueStatuts = Array.from(new Set(allDepartures
        .map(d => d.statut)
        .filter(s => s && s.trim() !== '')
      )).sort();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await (departService as any).import(formData);
  ...
  const handleExport = async () => {
    setIsLoading(true);
    try {
      await (departService as any).export();
