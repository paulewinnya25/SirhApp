...
const ProcedureTracking = () => {
  ...
  const [newDossierData, setNewDossierData] = useState<any>({
    nom: '',
    prenom: '',
    email: '',
    telephone: '',
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const specialitesUniques = Array.from(new Set(dossiersResponse.dossiers.map(d => d.specialite).filter(Boolean)));
        setSpecialites(specialitesUniques);
        
        // Compter les nouveaux dossiers (créés aujourd'hui) - réservé pour usage futur
