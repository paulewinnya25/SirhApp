...
const EmployeeRequests = () => {
  ...
  const [sortConfig, setSortConfig] = useState<any>({ key: 'request_date', direction: 'desc' });
  ...
  const handleExport = async (format) => {
    ...
    try {
      if (format === 'csv') {
        await (requestService as any).exportToCSV(filteredRequests);
      } else if (format === 'excel') {
        await (requestService as any).exportToExcel(filteredRequests);
      } else if (format === 'pdf') {
        await (requestService as any).exportToPDF(filteredRequests);
      }
