...
const MedicalVisits = () => {
  ...
  const [sortConfig, setSortConfig] = useState<any>({ key: 'date_prochaine_visite', direction: 'asc' });
  ...
  const calculateStats = useCallback((visits) => {
    ...
    const days30 = visits.filter(v => {
      const nextVisit = new Date(v.date_prochaine_visite);
      const diffDays = Math.ceil(((nextVisit as any) - (today as any)) / (1000 * 60 * 60 * 24));
  ...
  const fetchVisites = useCallback(async () => {
    ...
    try {
      ...
      const uniquePostes = Array.from(new Set(visitesData.map(v => v.poste).filter(Boolean))).sort();
    ...
    return [...filteredVisites].sort((a, b) => {
      ...
      if (['date_derniere_visite', 'date_prochaine_visite'].includes(sortConfig.key)) {
        ...
        return sortConfig.direction === 'asc' 
          ? (dateA as any) - (dateB as any) 
          : (dateB as any) - (dateA as any);
      }
  ...
  const getStatusBadge = useCallback((status, nextVisitDate) => {
    ...
    const diffDays = nextVisit ? Math.ceil(((nextVisit as any) - (today as any)) / (1000 * 60 * 60 * 24)) : null;
